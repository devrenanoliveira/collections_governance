...
- `assessorias` — third-party agency commissioning: `comissaoBase` by aging band, `distribuicaoCarteira` (portfolio share by agency/band), `comissaoIndireta`, `concorrencial`, `rituais`, `regulamento` (path to a standalone `regulamento.html` file, same convention as `estudos[].arquivo` — shown via an in-tab toggle next to "Comissionamento" in `renderAssessorias()`, loaded lazily into `#regulamentoFrame` on first click; the button's visible label is "Política de Perde/Ganha", intentionally different from the `regulamento` key name and from the file's own `<h1>` — don't "fix" the mismatch, it's the requested UI wording). Unlike the "Estudos" tab's iframe, `#regulamentoFrame` is **not** wrapped in a `.chart-card` — that wrapper doubled up with `regulamento.html`'s own bordered/padded layout into a visible "box inside a box", so it was dropped here on purpose (2026-09-22); don't re-add it without checking that regression first. The commission-multiplier matrix (`multiplicadores`, by attainment-of-target tier) was discontinued 2026-09-22 — removed from data and rendering, not just hidden.
