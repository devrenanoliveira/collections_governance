...
- `assessorias` — third-party agency commissioning: `metas`, `comissaoBase` by aging band, `comissaoIndireta`, `concorrencial` (just `numeroPlayers`/`periodoEstabilidade` — no numbers about share or redistribution, see below), `rituais`, `regulamento` (path to a standalone `regulamento.html` file, same convention as `estudos[].arquivo`).

  **`renderAssessorias()` (2026-09-22 rewrite) splits the tab into 4 non-overlapping toggles** — `assessoriasMostrarVista(vista)`, vistas `metas`/`share`/`premiacao`/`rituais` — instead of the earlier 2-toggle "Comissionamento vs. Regulamento" split. That split had put the SAME fact (portfolio share by agency/band, and the trimestral redistribution rule) in both `data.json` and `regulamento.html`, and the two had drifted to genuinely different numbers (a real data bug, caught 2026-09-22, not just a UI nit). The fix wasn't to reconcile both copies — it was to delete `assessorias.distribuicaoCarteira` and the `redistribuicao` string from `concorrencial` entirely, so `regulamento.html` (section `#portfolio`, current + planned-Q4/2026 tables, and section `#portfolio`'s sibling "Perde e Ganha" rule) is now the **only** place either fact lives. Do not re-add a share table or a redistribution-rule sentence to `data.json`/`index.html` — that would recreate the exact bug this rewrite fixed.

  The 4 vistas: **Metas & Comissão** (data-driven: `metas`, `tblComissaoBase`, `comissaoIndireta` — unique to the dashboard, no regulamento overlap) · **Share & Redistribuição** (`concorrencial` panel, data-driven but deliberately thin — no numbers — plus `#shareFrame` anchored at `regulamento.html#portfolio`) · **Premiação & Bônus** (`#premiacaoFrame` anchored at `regulamento.html#premiacao`, no dashboard-side equivalent) · **Rituais & Governança** (`rituaisGrid`, data-driven, plus `#governancaFrame` anchored at `regulamento.html#governanca` for the repasse/prestação-de-contas rules). Each iframe's `src` is `produto().assessorias.regulamento + '#' + ancora` (see `ASSESSORIAS_FRAME_POR_VISTA` in `index.html`) — a plain URL hash into the SAME standalone file, not three separate files; the browser's native anchor-scroll does the rest, no JS scrolling code needed. None of the 4 iframes/panels is wrapped in `.chart-card` (see the "box inside a box" note below).

  `regulamento.html` itself has two portfolio-share tables now: **1.1 "vigente"** (current, must match reality — as of 2026-09-22: Fácil 95%/90%, Decisão 5%/5% in the 66-180/>180 bands) and **1.2 "expectativa a partir do último trimestre de 2026"** (Fácil 85%/80%, Decisão 15%/15% — an unconfirmed plan, not yet in effect). Keep both, don't collapse them into one table again — that's exactly what caused the earlier drift (an old draft number sitting in a table labeled as if it were current). The commission-multiplier matrix (`multiplicadores`, by attainment-of-target tier) was discontinued 2026-09-22 — removed from data and rendering, not just hidden. Unlike the "Estudos" tab's iframe, none of `assessorias`'s iframes are wrapped in a `.chart-card` — that wrapper doubled up with `regulamento.html`'s own bordered/padded layout into a visible "box inside a box", so it was dropped here on purpose (2026-09-22); don't re-add it without checking that regression first.
