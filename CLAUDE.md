...
8. **"Mapa da Cobrança" tab (added 2026-09-25, 9th tab)** is fully static — no `render*()` function, no `data.json`/`produto()` dependency — just a `.recurso-card`/`.recurso-btn` (same pattern as the Régua de Cobrança tab's reference links) linking out to a separate Claude artifact (`https://claude.ai/artifact/...`) that tracks group-wide PF/PJ collections-governance progress, out of scope for this Z-ON-specific dashboard. **Don't try to `<iframe>` it** — claude.ai sends `frame-ancestors 'self' ...`, so any third-party site framing it renders blank; this was tried and reverted. Link-out is correct here, not a fallback.
...
- `assessorias` — third-party agency commissioning: `metas`, `comissaoBase` by aging band, `comissaoIndireta`, `concorrencial` (just `numeroPlayers`/`periodoEstabilidade`), `portfolioVigente` (share por faixa/assessoria + `diretrizes` textuais, faixas em `faixas`), `portfolioExpectativa` (mesmo formato, sem `faixas` própria — reaproveita `portfolioVigente.faixas` na renderização —, mais `confirmado: false`, `vigenciaInicio`, `nota`), `perdeGanha` (`regraGeral`, `travaAncora`, `amostragemProporcional`), `premiacao` (`mensal`/`trimestral`, cada um com `criterioBase`/`criterioTurbinado`/`lugares[]` de `{posicao,base,turbinado}`, mais `bonusPrejuizo`, `travaMinima`, `formaPagamento`), `governancaRepasse` (`repasseMinimo`, `prestacaoContas`), `rituais`.

  **Todo o conteúdo do antigo `regulamento.html` foi incorporado nativamente ao `data.json`/`index.html` em 2026-09-22** — não existe mais um documento HTML separado nem iframe nessa aba. Motivo: o arquivo standalone vivia num `<iframe>` com seu próprio `<html>`, então o dark mode do dashboard (`toggleDark()`, classe `body.dark`) nunca alcançava o conteúdo lá dentro — ficava sempre claro, com fonte serifada diferente do resto do site. A escolha consciente aqui foi abrir mão da portabilidade de "arquivo único que abre sozinho fora do dashboard" (que os `estudo-*.html` ainda têm, de propósito, porque aqueles são estudos pontuais que fazem sentido isolados) em troca de consistência visual e de tema — o regulamento é conteúdo de referência que o usuário abre o tempo todo dentro do dashboard, não um documento pra circular fora dele. Se um dia precisar de uma versão exportável/impressa do regulamento, seguir o padrão do `calculadora-desconto.html` (snapshot gerado a partir dos mesmos dados, regenerado manualmente após qualquer edição) em vez de recriar o iframe.

  `renderAssessorias()` divide a aba em 4 vistas não sobrepostas via `assessoriasMostrarVista(vista)` (`metas`/`share`/`premiacao`/`rituais`), usando `portfolioTableHtml()`, `prizeCardsHtml()` e `criteriosHtml()` como helpers de template. `governancaRepasse` renderiza dentro da vista `premiacao` (botão "Premiação & Bônus"), não em `rituais`

  **Botão "Relatório para as assessorias" (24/09/2026)**: `exportarRelatorioAssessorias()`, no topo da aba, monta as 4 vistas (Metas & Comissão, Share & Redistribuição, Premiação & Bônus, Rituais de Gestão) num único documento HTML standalone (`window.open('', '_blank')` + `document.write()`), com botão interno "🖨️ Imprimir / Salvar PDF" (`window.print()`, escondido via `@media print`). Segue o padrão já usado em `zon-dashboard-powered/script.js` (`exportarRelatorioAssessorias()`/`_relAssHTML()`), mas simplificado — sem planilhas de performance por assessoria, porque aqui os dados de `assessorias` são política/governança estática, não métricas comparativas. Reaproveita `portfolioTableHtml()`, `prizeCardsHtml()`, `criteriosHtml()`, `brl()`, `pct()` — **não duplica** lógica de formatação nem dado de `portfolioVigente`/`portfolioExpectativa`, respeitando o aviso abaixo. — é sobre como o dinheiro da premiação é repassado/prestado contas, não sobre os rituais de gestão diária/semanal/mensal (movido 2026-09-22, a pedido do usuário; o botão da vista `rituais` virou só "Rituais de Gestão"). Isso substituiu um split anterior (2-toggle "Comissionamento vs. Regulamento", depois um de 4 toggles com 3 iframes ancorados) que tinha um bug de dado real: a mesma informação (share por assessoria, regra de redistribuição trimestral) vivia duplicada em `data.json` e no HTML do regulamento, e as duas cópias já tinham divergido para números diferentes. **Não recriar uma segunda cópia de `portfolioVigente`/`perdeGanha` em outro lugar do dashboard** — isso reabriria exatamente esse bug.

  `portfolioVigente` × `portfolioExpectativa` são dois estados no tempo do mesmo fato, não uma duplicata: o primeiro é o share vigente (2026-09-22: Fácil 95%/90%, Decisão 5%/5% nas faixas 66-180/>180), o segundo é uma expectativa **não confirmada** (`confirmado: false`) cogitada pra começar no último trimestre de 2026 (Fácil 85%/80%, Decisão 15%/15%). Manter os dois campos separados — colapsar em um só foi o que causou a divergência anterior (um rascunho futuro escrito como se fosse o valor atual). A matriz de multiplicador de comissão (`multiplicadores`, por faixa de atingimento) foi descontinuada em 2026-09-22 — removida de dado e renderização, não só escondida.
